...
class MyController extends Controller {
    ...
    public  function  __construct()
    {
        ...
        if(CONTROLLER_NAME!="Install") // 如果当前访问的控制器是 安装控制器
        {
            // 判断是否安装过
    ...
    protected function is_sgin()
    {
        ...
        {
            is_login($this->u_id,$this->s_pix);//验证是否登录
        }//------------------验证登录页面是否已经登录了
    }
     ...
     protected function read_limit($f,$limit_id) //读取权限时，如果不存在权限文件创建
    {
        if(!file_exists($f))
        {
            ...
            {
                echo "读取权限文件失败";
                write_log("读取权限时，不存在权限文件创建权限文件失败---提示信息：读取权限文件失败");
            }
        }
    }
    ...
    protected function rw_identity($user,$role,$r_id)
    {
        ...
        {
            $_SESSION[$this->u_id.PRO_W]=PRO_W;
            $meg=$user->getFieldById($this->u_id,"meg");
            if($meg>0)
            {
                S($this->u_id."pro_w_n",$meg,T_INT);
            }
        }//---如果有添加任务的权限 取得消息个数

        if($this->is_admin($user) || $task_warn==2)
        {
            $_SESSION[$this->u_id.TAST_W]=TAST_W;
            $c=$this->u_task_c("c");
            if(isset($c) && $c>0)
            {
                S($this->u_id."tast_w_n",$c,T_INT);  // 执行任务权限 待执行任务个数
            }
        }
    }
    ...
    {
        if(isset($get["bumen"]) && !empty($get["bumen"]) && in_array($get["bumen"],$bu_men))
        {
         //   $w["bumen"]=array("eq",$get["bumen"]);
            $bm_u=$this->bm_search($get["bumen"],$bu_men);
            if(isset($bm_u) && !empty($bm_u))
            {
                $w["u_id"]=array("in",$bm_u);
            }else
            {
                $w["u_id"]=array("eq",0);  // 如果没有数据
            }
    ...
    {
        $get_bumen=add_slashes($get_bumen);
    ...
    {
        if(isset($_SESSION[$this->u_id.PRO_W]) && $_SESSION[$this->u_id.PRO_W]=PRO_W) // 信息反馈
        {
           if(!S($this->u_id."pro_w_n"))
           {
               $user=D("User");
               $meg=$user->getFieldById($this->u_id,"meg");
               if($meg>0)
               {
                   S($this->u_id."pro_w_n",$meg,300);
                   $this->assign("meg",$meg);
               }
           }else
           {
               $this->assign("meg",S($this->u_id."pro_w_n"));
           }
        }
        if(isset($_SESSION[$this->u_id.TAST_W]) && $_SESSION[$this->u_id.TAST_W]=TAST_W)  //待执行任务
        {
            if(!S($this->u_id."tast_w_n"))
            {
                $c=$this->u_task_c("c");     //待执行任务个数
                if(isset($c) && $c>0)
                {
                    S('task_w_n',$c,300);
                    $this->assign("p_c",S($this->u_id."tast_w_n"));
                }
            }else
            {
                $this->assign("p_c",S($this->u_id."tast_w_n"));
