    ...
    <form method="post" class="form-x" action="/Back/Pass/index">
        ...
        <div class="form-group">
            ...
            <div class="field">
                ...
                &nbsp;
                <a class="button bg-main " style="margin-top: 15px;" href="javascript:;" onclick="email_send('<?php echo ($id); ?>','<?php echo ($mail); ?>')">发送验证</a>
            </div>
        </div>
        <div class="form-group">
            <div class="label">
...
<style>
    ...
    .close{ position: absolute;color:#fff;width:28px; height:28px;top:8px;right:10px; cursor: pointer;}
    ...
    .input a,a.input{color:#0F85D4;}
        ...
        <script>
           /* $(".close").click(function(){
            ...
            {
                $.post("/Back/Pass/emailSend",{'id':id,'mail':mail},function(data){
                    console.log(data);
                    ...
                    switch(data)
                    {
                        ...
                        case "mail_state_no":
                            ...
                            $('.no-skin').find("p").html("<a href='/Back/Pass/index'>由于您长时间未操作<br/>请点击返回第一步</a>")
                        case "timeout":
                            $('.no-skin').find("h1").html("返回第一步");
                            $('.no-skin').find("p").html("<a href='/Back/Pass/index'>2由于您长时间未操作<br/>请点击返回第一步</a>")
                        break;
                        case "user_is_no":
                    ...
                    // 点击关闭按钮失效
                    $(".close").click(function(){
                        $('.no-skin').stop(true,true).delay(0).fadeOut();
                    });
                    $('.no-skin').delay(4000).fadeOut();
                });//发送邮件
				
				  //功能：根据用户输入的Email跳转到相应的电子邮箱首页--- 只在当前页面起作用，放入引用js 中不起作用
