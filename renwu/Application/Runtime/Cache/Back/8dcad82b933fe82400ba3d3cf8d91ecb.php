...
<div class="panel admin-panel">
  <div class="body-content">
      <ul>
          <?php if(!empty($operate)): ?><li>其它系统操作: </li>
              <?php if(is_array($operate)): foreach($operate as $key=>$v): ?><li><a href="javascript:;" onclick="oldf('<?php echo ($v["execs"]); ?>')"><?php echo ($v["n"]); ?></a></li><?php endforeach; endif; endif; ?>
          <hr/>
      </ul>
...
<script>
    function oldf(action) // 执行清理文件
    {
        $.post('/Back/Sysset/'+action,'',function(data){
            if(data=="empty")
            {
                alert("没有要清理的文件");
            }
