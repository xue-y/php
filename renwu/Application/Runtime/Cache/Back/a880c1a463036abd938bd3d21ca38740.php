  ...
  <div class="body-content">
    <div class="form-x">
      <div class="form-group">
        ...
        <div class="form-group">
            <label class="label">问题反馈：</label>
            <p><?php if(isset($info["meg"])): ?><a class="input w50" title="查看问题反馈" href="/Back/Personal/megList?id=<?php echo ($info["id"]); ?>">您提交的问题反馈<span class="red"> <?php echo ($info["meg"]); ?> </span>条信息</a>
                <?php else: ?>   <span class="input w50">暂时没有反馈信息</span><?php endif; ?>
            </p>
        </div>
        <?php if(($info["warn"]) > "0"): ?><div class="form-group">
                <label class="label">任务提醒：</label>
                <p><a class="input w50" title="查看新任务" href="/Back/Task/index?p_id=<?php echo ($info["p_id"]); ?>">新任务<span class="red"> <?php echo ($info["warn"]); ?> </span>条提醒</a></p>
            </div><?php endif; ?>
    </div>
  </div>
</div>
                ...
                function email_send(t,id,mail,state)//执行邮箱验证--发送验证
                {
                    ...
                    $.post("/Back/Personal/emailSend",{'id':id,'mail':mail,'state':state},function(data){
                      //  console.log(data)  // 取得发送邮件后是否成功
                        ...
                        if(data=="send_mail_c")
                        {
                            $('.no-skin').find("h1").html("今天邮件发送超过5次");
