...
class UserModel extends Model{
    public  $_validate=array(
       array('u_name','require','用户名必填'),
        ...
        array('u_pass2','u_pass','确认密码不正确',2,'confirm'),  //用户登录  添加 修改
        // 附加表验证
        array('phone','/^1(\d){10}$/','请输入正确的手机号',2,'regex'),
        array('wx','/^[\s\S]{3,60}$/','微信号字符长度3--20个字符',2,'regex'),
        array('info','/^[\s\S]{1,80}$/','微信号字符长度80个字符',2,'regex'),
    );
    public $_auto=array(
    ...
    public function  found_all()
    {
       ...
       $sql="select `id`,`u_name` from `{$table_prefix}user` where `id` in ( SELECT `found` FROM __USER__ GROUP BY `found`)";
    ...
    {
       return $this->field("id,u_name,bumen,role_id,mail,is_jihuo")->find($u_id);
    }
    ...
    /**查询所有的咨询人员
     * */
    public function all_zx()
    {
      // return $this->where("role_id=".ZIXUN)->field("id,u_name")->select();
	  return $this->field("id,u_name")->select(); // 取得所有管理员
    }
     ...
     * @return 返回用户附加表信息
     * */
    public function user_fj($id)
    {
        return $this->table("__USER_FJ__")->field("id",true)->find($id);
    }
}