...
<body class="line-add">
<h3>
    <a   onclick="javascript :history.back(-1)" >←</a> | 修改推荐 <a class="float-right" href="/Home/Line/index">推荐</a>
</h3>
<p class="red">值为空，默认不修改</p>
<?php if(($line_info["state"]) != "1"): ?><p class="red">此推荐人已经被审核过，不可修改</p><?php endif; ?>

<form <?php if(($line_info["state"]) == "1"): ?>action="/Home/Line/execUate" method="post"<?php endif; ?> >
    ...
    <div class="form-group">
        <div class="field">
            <input type="text" class="input w100" name="n"  placeholder="请输入好友姓名" data-validate="title:用户名长度2--10个字符"  value="<?php echo ($line_info["n"]); ?>" required="required" />
            <div class="tips"></div>
        </div>
    </div>
    <div class="form-group">
